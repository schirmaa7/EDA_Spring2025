...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    x=0,
    y=0.025, #tell R where on the plot you want your text to go
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6')) #expression command allows mu and squared symbology, paste function combines all objects together in order to make sure they stay together
    )
...
PeterPaul.chem.nutrients %>%
  ...
  labs(title="number of lake observations each month", x="month", y="observation count")
...
PeterPaul.chem.nutrients%>%
  ggplot()+
  geom_bar(aes(x=factor(month, levels=1:12, labels=month.abb)))+ #add month labels to X axis
  labs(title="number of lake observations each month", x="month", y="observation count")
...
PeterPaul.chem.nutrients%>%
  ...
  scale_x_discrete(drop=F) #ask R to not drop missing months
  labs(title="number of lake observations each month", x="month", y="observation count")
...
PeterPaul.chem.nutrients%>%
  ...
  labs(y="surface temperature", x="day number", title="Surface temperature by day\n in Peter Lake and Paul Lake")
...
EPAair%>%
  ...
  geom_point()+
  geom_smooth(method = lm, se=FALSE) #line of best fit, method = lm means linear regression model
...
#also could do violin plots and histograms to understand variability between one year and the other
