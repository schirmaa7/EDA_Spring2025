...
```{r project.setup, warning=FALSE, message=FALSE, echo=TRUE}
remove(list=ls())
#Import libraries
library(tidyverse)
library(lubridate)
...
```{r data.summary, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r ozone 2018 boxplot, warning=FALSE, message=FALSE, echo=FALSE, fig.align='left', fig.height=4, fig.cap='Ozone Levels by Month in 2018'}
epa_data%>%
  filter(Year==2018)%>%
...
```{r ozone 2019 boxplot, warning=FALSE, message=FALSE, echo=FALSE, fig.height=4, fig.cap='Ozone Levels by Month in 2019'}
epa_data%>%
  filter(Year==2019)%>%
  ggplot(aes(x=factor(Month, levels=1:12, labels=month.abb), y=Ozone))+
...
https://github.com/schirmaa7/EDA_Spring2025.git
